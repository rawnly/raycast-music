...
import { createDeeplink, handleResult, promisify } from "@/lib/utils";
...
const createQuickLink = (action: QuickAction) =>
  createDeeplink('quick-actions', { action: action.id })
...
export default function QuickActions() {
  ...
  const quickActions: QuickAction[] = [
    {
      ...
      onAction: handleResult(
        music.scripts.player.playpause,
        {
          onComplete: revalidatePlayerState,
      ...
      onAction: handleResult(
        music.scripts.currentTrack.love(!currentTrack?.loved),
        {
      ...
      onAction: handleResult(
        music.scripts.currentTrack.dislike(false),
        {
      ...
      onAction: handleResult(
        music.scripts.player.volume.increase(),
        {
      ...
      onAction: handleResult(
        music.scripts.player.volume.decrease(),
        {
          errorMessage: 'Failed to increase volume',
          successText: 'Increased volume'
        }
      ...
      onAction: handleResult(
        music.scripts.currentTrack.reveal,
        {
  ...
  return (
    <List isLoading={isLoadingPlayerState || isLoadingAuth || isLoadingCurrentlyPlaying} navigationTitle='Quick Actions'>
      <List.EmptyView title="Cannot find whay you're looking for" />
      {quickActions.map((action) => (
        <List.Item key={action.id} title={action.title} icon={action.icon} subtitle={action.description} actions={
          <ActionPanel>
            <PerformAction playerState={playerState ?? PlayerState.STOPPED} action={action} />

            <Action.CreateQuicklink title="Create Quicklink" quicklink={{
...
const PerformAction = ({ action, playerState }: { action: QuickAction; playerState: PlayerState }) => {
  ...
  const warn = match(action.disabled)
    ...
    .with(P.string, msg => () => confirmAlert({
      title: 'Cannot perform action',
      message: msg,
      icon: Icon.Warning,
      primaryAction: {
  ...
  if (QuickAction.isView(action)) {
    return <Action.Push {...action} target={action.view()} onPush={() => debug('View pushed', action.id)} />
  }
